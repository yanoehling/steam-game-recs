...
function FriendButton({friend}){
    ...
    async function pega_recomendas(){
        let user_data = await fetch(`https://steamrecommendationsserver.onrender.com/users/me`, {
            method: "GET",
            headers: {
        ...
        for (let a of user.friends) {
            let amigo_data = await fetch(`https://steamrecommendationsserver.onrender.com/users/${a.id}`, {
                method: 'GET',
                headers: {
                'Content-Type': 'application/json',
                }
            ...
            if (amigo.username === friend.username){
                for (let r of a.recommendations) {
                    let game_data = await fetch(`https://steamrecommendationsserver.onrender.com/games/${r}`, {
                        method: 'GET',
                        headers: {
                            'Content-Type': 'application/json',
                        }
...
function SearchBarTag({type, onclick}){
    ...
    const adicionar_amigo = async () => {
        ...
        let data_register = await fetch(`https://steamrecommendationsserver.onrender.com/users/add/friend`, {
            method: "POST",
            headers: {
                'Content-Type': 'application/json',
                "Authorization": `Bearer ${TOKEN}`
    ...
    const remover_amigo = async () => {
        ...
        const jsonData = JSON.stringify({friendName: amigo})
        let data_register = await fetch(`https://steamrecommendationsserver.onrender.com/users/remove/friend`, {
            method: "DELETE",
            headers: {
                'Content-Type': 'application/json',
                "Authorization": `Bearer ${TOKEN}`
