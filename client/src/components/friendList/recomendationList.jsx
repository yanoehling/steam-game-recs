...
function FriendButton({friend, id, type}){
    ...
    async function add_recomendas(){
        ...
        for (let a of user.friends) {
            ...
            if (amigo.username === friend.username){
                let recomendacao_data = await fetch(`https://steamrecommendationsserver.onrender.com/users/add/recommendation`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        "Authorization": `Bearer ${TOKEN}`
    ...
    async function remove_recomendas(){
        ...
        for (let a of user.friends) {
            ...
            const amigo = await amigo_data.json();
            if (amigo.username === friend.username){
                let recomendacao_data = await fetch(`https://steamrecommendationsserver.onrender.com/users/remove/recommendation`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        "Authorization": `Bearer ${TOKEN}`
